...
const AboutTwo = () => {
  ...
  useEffect(async () => {
    if (library) {
      ...
      contractInstanceOld = new library.eth.Contract(ContractAbiOld, contractAddressOld);
  ...
  const OnClickPayButton = async () => {
    //console.log({checked})
    const val = checked.length > 0 ? 0.001 * (checked.length * payMonth) : 0.001 * (myNodes * payMonth);
    await NodeManagementInstance.methods
      .pay(payMonth, checked)
      .send({ from: account, value: Web3.utils.toWei(val.toString(), 'ether') })
      .then(() => {
  ...
  const getNodesInfo = async () => {
    ...
    if (!hasRunNodeInfo) {
      ...
      if (numberOfNodes > 0) {
        ...
        if (nodesUser) {
          const tier = await NodeManagementInstance.methods
            .getTierByName("basic")
            .call()
            .catch((e) => {
              console.log({ e });
          ...
          for (let i = 0; i < nodesUser.length; i++) {
            ...
            let claimeableAmount =
              (((new Date().getTime() / 1000 -
                parseInt(nodesUser[i].claimedTime)) *
                (parseInt(nodesUser[i].tierIndex) === 0
                  ? tier.rewardsPerTime
                  : 0.8)) /
                tier.claimInterval) +
              parseInt(nodesUser[i].leftover);
            ...
            if (Number(claimeableAmount) > 0.0) {
              ...
              nodeElement.push(
                <tr key={i}>
                  <td>{NameList[i]}</td>
                  <td>{Number(claimablePXTforNode).toFixed(3)}</td>
                  ...
                  {/* <td><input type="checkbox" onClick={handleCheck} value={nodesUser[i].id ?? ''} /></td> */}
                  <td>READY</td>
              ...
              nodeElement.push(
                <tr key={i}>
                  ...
                  {/* <td><input type="checkbox" onChange={handleCheck} value={nodesUser[i].id ?? ''} /></td> */}
                  <td>WAIT 1HR</td>
        ...
        setCompoundAmount(parseInt(Number(r)/pxtPerNodeCreate));
  ...
  const getNodeCreationTime = async () => {
    const res = await NodeManagementInstance.methods
  ...
  const calcRewards = (node) => {
    ...
    if (diff <= 0) return 0
    let one = parseEther("1"), m = parseEther("1")
    if (node.multiplier) {
          ...
          <div className="container">
            <div className="about_inner">
              <div className="left">
                ...
                <br/>
                Multiple wallet tracker coming soon.
                {/*
                ...
                */}
                ...
                {/* <>
                  <span className="nodecreatecontainer">
                    <button onClick={() => setPayMonth(payMonth === 1 ? 1 : payMonth - 1) } className="btn minus">-</button>
                    <button className="btn" onClick={OnClickPayButton}>
                      <span>Pay {payMonth} Month{payMonth > 1 && 's'} ({checked.length === 0 ? 'All' : checked.length})</span>
                    </button>
                    <button onClick={() => setPayMonth(payMonth === 3 ? 3 : payMonth + 1) } className="btn plus">+</button>
                  </span>
                </> */}
              </div>
              ...
              <div className="right">
                ...
                >
                  ...
                  <p>
                    ...
                    </a> <i className="fa fa-exclamation-triangle"></i> 
                  </p>
                  <br />
                  ...
                  {(parseInt(pendingRewards/pxtPerNodeCreate) > 0) ? (
	                  ...
	                  Or compound&nbsp;&nbsp;
	                    <input
	                      ...
	                      id="compoundNumber"
	                      size="2"
	                      defaultValue={parseInt(pendingRewards/pxtPerNodeCreate)}
	                      onChange={(e) => setCompoundAmount(e.target.value)}
	                    />&nbsp;&nbsp;
	                    <button className="btn" onClick={OnClickCompoundButton}>New Nodes</button>
	                  </>
                  ) : (
	                  <>
	                  &nbsp;&nbsp; You need {(10-pendingRewards).toFixed(2)} more PXT to compound.
	                  </>
                  )}
                  <br/>
                  <br/>
                  ...
                  {(myNodes == 0 && myOldNodes > 0 && migratedNodes) ? (
                    <>
					  <br/>
					  <strong>Step 1:</strong> <a href="javascript:void(0)" onClick={OnClickMigrateNodesButton}>Migrate Nodes</a>
                    </>
                  	) : (
	                <>
	                  <br />
	                  <strong>Step 1:</strong> Complete <i class="fa fa-check"></i>
	                </>
                  )}
                  {(myOldNodes > 0 && migratedRewards) ? (
	                <>
	                  <br />
	                  <strong>Step 2:</strong> <a href="javascript:void(0)" onClick={OnClickMigrateRewardsButton}>Migrate Rewards</a>
	                  {/*<strong>Step 2:</strong> <a href="javascript:void(0)" className="strikethrough">Migrate Rewards</a> (Temporarily disabled)*/}
	                </>
                  ) : (
				   	<>
	                  <br/>
	                  <strong>Step 2:</strong> Complete <i class="fa fa-check"></i>
					</>
                  )}
                  {migratedTokens && (
                    <>
                      <br />
                      <strong>Step 3:</strong> <a href="javascript:void(0)" onClick={OnClickTokenButton}>Migrate Tokens</a>
                    </>
                  )}
                </div>
              </div>
